...
namespace ChatBot.Services
{
    ...
    public class AzureAuthenticationService
    {
        ...
        private const string ServiceUrl = "https://{0}.api.cognitive.microsoft.com/sts/v1.0/issueToken";
        ...
        /// </remarks>
        public static async Task<string> GetAccessToken(string key, string region)
        {
            ...
            if (_tokens.ContainsKey(key))
            {
                ...
                if ((DateTime.Now - azureAuthToken.LastUpdateTime) > TokenCacheDuration)
                {
                    ...
                    azureAuthToken.StoredTokenValue = await GenerateToken(key, string.Format(ServiceUrl, region));
                }
            }
            else
            {
                ...
                var tokenValue = await GenerateToken(key, string.Format(ServiceUrl, region));
